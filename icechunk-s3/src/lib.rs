...
impl S3Storage {
    ...
    pub fn new(
        ...
        if legacy_rooted_keys && !prefix.is_empty() {
            return Err(other_error(
                "legacy_rooted_keys is only valid with an empty prefix",
            ));
        }
        // Forcing the layout pre-seeds the cell, so `probe_layout` is never reached.
        let key_layout = if legacy_rooted_keys {
...
/// Resolve the layout from per-anchor `(clean_etag, rooted_etag)` HEAD results.
///
...
/// from one that genuinely holds distinct objects at both keys (→ mixed). Pure,
/// so the normalization handling can be unit-tested without a network round-trip.
fn layout_from_anchor_etags(
    anchors: &[(Option<String>, Option<String>)],
    bucket: &str,
) -> StorageResult<KeyLayout> {
    let mut clean_seen = false;
    let mut rooted_seen = false;
    for (clean, rooted) in anchors {
        match (clean, rooted) {
            // Same physical object under both keys: the store normalized away the
            // leading slash. This is the clean layout, not a mixed repository.
            (Some(c), Some(r)) if c == r => clean_seen = true,
            // Genuinely distinct objects at both keys: ambiguous; flag as mixed.
            (Some(_), Some(_)) => {
                clean_seen = true;
                rooted_seen = true;
            }
            (Some(_), None) => clean_seen = true,
            (None, Some(_)) => rooted_seen = true,
            (None, None) => {}
        }
    }
...
fn decide_layout(clean: bool, rooted: bool, bucket: &str) -> StorageResult<KeyLayout> {
    match (clean, rooted) {
        // Both layouts present: ambiguous and unsafe
        (true, true) => Err(other_error(format!(
        ...
        (false, true) => Ok(KeyLayout::LegacyRoot),
...
mod tests {
    ...
    fn test_decide_layout_four_way() {
        ...
        assert_eq!(decide_layout(false, false, "b").unwrap(), KeyLayout::Standard);
        // clean repo -> clean
        assert_eq!(decide_layout(true, false, "b").unwrap(), KeyLayout::Standard);
        // legacy rooted repo -> legacy
        assert_eq!(decide_layout(false, true, "b").unwrap(), KeyLayout::LegacyRoot);
        // mixed -> hard error
        let err = decide_layout(true, true, "mybucket").unwrap_err();
    ...
    fn test_layout_from_anchor_etags() {
        ...
        let layout = |anchors: &[(Option<String>, Option<String>)]| {
            layout_from_anchor_etags(anchors, "b")
        };

        // empty/new repo: nothing found -> clean
        assert_eq!(layout(&[(None, None)]).unwrap(), KeyLayout::Standard);
        // clean repo (clean key found, rooted key absent: AWS 404 or rustfs 400)
        assert_eq!(layout(&[(et("E"), None)]).unwrap(), KeyLayout::Standard);
        // genuine legacy rooted repo: only the rooted key exists
        assert_eq!(layout(&[(None, et("E"))]).unwrap(), KeyLayout::LegacyRoot);
