//! `ListObjectsV2` pages signed with `aws-sigv4` and scanned with `memchr` for `<Size>`.
//! The SDK parser allocates per object, which is most of the CPU on large listings.
...
impl SignedLister {
    pub(crate) fn new(
        ...
        // Dotted bucket names break the virtual-host TLS certificate, as in the SDK.
        let use_path_style =
...
/// Scans each body chunk as it arrives; buffering the page first cost ~25% CPU.
#[async_trait]
impl ListPageFetcher for SignedLister {
    async fn attempt_page(&self, list_prefix: &str, token: Option<&str>) -> PageAttempt {
        match self.send_once(list_prefix, token).await {
            ...
            // A credential rotation fails every in-flight request. Refresh and retry once
            // here, so the engine does not read it as congestion and halve concurrency.
...
/// `AuthExpiry` is retried once inside the attempt, after a credential refresh.
enum Outcome {
    Attempt(PageAttempt),
...
impl SignedLister {
    ...
    async fn classify_response(
        ...
        // Expired session credentials come back as 400 `ExpiredToken` or 403.
...
/// The SDK client also retries 408 and 499 (Tigris). GCS and Azure do not.
fn is_s3_extra_retryable_status(status: u16) -> bool {
    status == 408 || status == 499
}

/// `ExpiredToken` also matches `ExpiredTokenException` from some S3-compatible stores.
...
/// Refreshable and environment credentials are cached and refreshed on expiry.
#[derive(Debug)]
enum Signer {
    Unsigned,
...
impl DynamicSigner {
    ...
    /// Holding the async lock across the fetch makes the refresh single-flight.
    ...
    /// Clears the cache only if no other request refreshed past `used_generation`.
...
/// Honors `AWS_ENDPOINT_URL[_S3]` like the SDK, or the fast path lists a different host.
...
/// Parsed like the SDK's `parse_bool`: only case-insensitive `true`, not `1`.
...
/// The SDK resolves FIPS and dualstack hosts from the environment. The signed lister
/// cannot, so those configurations use the normal listing.
...
/// A dot breaks the `*.s3.<region>.amazonaws.com` wildcard certificate.
...
fn build_http_client(settings: &Settings) -> StorageResult<reqwest::Client> {
    ...
    if let Some(timeouts) = settings.timeouts() {
        ...
        // The engine owns retries, so one reqwest request is one attempt.
...
/// A match can straddle a chunk boundary by up to this many bytes.
const TAIL_MAX: usize = TOKEN_OPEN.len() - 1;

/// Real tokens are under 1 KB. A longer capture means a corrupt body.
...
/// Incremental page scanner. Matches can straddle chunks, and element order varies by store.
/// Element text has no literal `<`, so value bytes kept in the tail never start a match.
#[derive(Debug, Default)]
struct PageScan {
    mode: Mode,
    total: u64,
...
impl PageScan {
    ...
    /// `IsTruncated` without a usable token is an error, not the last page, which would
    ...
    /// Handles the one match that can start in the tail and end in this chunk.
    ...
    /// Stops the truncation and token finders once settled: rescanning every chunk for
    /// them cost more CPU than the whole-page scan.
...
enum PageEnd {
    ...
    /// `IsTruncated` was set but the token was missing, too long, or unterminated.
    TruncatedWithoutToken { bytes: u64, cause: &'static str },
}

/// A body read error mid-page is returned so the attempt is retried.
...
mod tests {
    ...
    async fn transient_507_is_retried_then_succeeds() {
        ...
        let total = sum(Arc::new(lister), &[("chunks".to_string(), false)], 3).await;
