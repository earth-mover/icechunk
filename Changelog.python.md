...
- Add `hf_storage`, a preset for Hugging Face Storage Buckets. It takes the bucket's `namespace` and sets the gateway endpoint, the region and path-style URLs. The gateway discards user metadata, so Icechunk cannot recover from a lost response to a conditional write. Set `unsafe_use_metadata=False` in `StorageSettings` to stop sending that metadata ([#2346](https://github.com/earth-mover/icechunk/pull/2346)).
...
- S3 endpoints whose URL includes a path, such as Hugging Face Storage Buckets' `https://s3.hf.co/<namespace>`, no longer fail with `NoSuchBucket`. The AWS SDK joins the endpoint path and the bucket name without a separator, so requests addressed `/<namespace><bucket>/<key>`. Icechunk now appends the missing `/`. Endpoints without a path, such as Tigris, R2 and MinIO, keep the same behavior ([#2346](https://github.com/earth-mover/icechunk/pull/2346)).
