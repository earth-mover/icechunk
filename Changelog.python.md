...
- S3 endpoints whose URL includes a path, such as Hugging Face Storage Buckets' `https://s3.hf.co/<namespace>`, no longer fail with `NoSuchBucket`. The AWS SDK joins the endpoint path and the bucket name without a separator, so requests addressed `/<namespace><bucket>/<key>`. Icechunk now appends the missing `/`. Endpoints without a path, such as Tigris, R2 and MinIO, keep the same behavior.
