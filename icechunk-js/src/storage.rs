...
impl JsStorage {
    ...
    pub fn new_s3(
        ...
        let storage =
            icechunk::storage::new_s3_storage(opts, bucket, prefix, creds, false)
                .map_napi_err()?;
        Ok(JsStorage(storage))
    }
    ...
    pub fn new_r2(
        ...
        let storage = icechunk::storage::new_r2_storage(
            opts, bucket, prefix, account_id, creds, false,
        )
        .map_napi_err()?;
        Ok(JsStorage(storage))
    }
    ...
    pub fn new_s3_with_refreshable_credentials(
        ...
        let storage =
            icechunk::storage::new_s3_storage(opts, bucket, prefix, Some(creds), false)
                .map_napi_err()?;
        Ok(JsStorage(storage))
    }
