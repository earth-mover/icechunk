// napi-rs-artifact-metadata:{"version":2,"rootEntry":"index.js","exports":["Repository","JsRepository","Session","JsSession","Storage","JsStorage","Store","JsStore","CompressionAlgorithm","JsCompressionAlgorithm"],"managedRootEntries":["browser.js","index.js","icechunk.wasm","icechunk.debug.wasm"]}
/* eslint-disable */
/* prettier-ignore */
...
let __wasmFilePath = __nodePath.join(__dirname, 'icechunk.wasm32-wasi.wasm')
const __wasmDebugFilePath = __nodePath.join(__dirname, 'icechunk.wasm32-wasi.debug.wasm')

if (__nodeFs.existsSync(__wasmDebugFilePath)) {
  __wasmFilePath = __wasmDebugFilePath
} else if (!__nodeFs.existsSync(__wasmFilePath)) {
  const __wasiPackageEntry = require.resolve('@earthmover/icechunk-wasm32-wasi')
  const __packagedWasmFilePath = __nodePath.join(
    __nodePath.dirname(__wasiPackageEntry),
    'icechunk.wasm32-wasi.wasm',
  )
  if (!__nodeFs.existsSync(__packagedWasmFilePath)) {
    throw new Error(
      '@earthmover/icechunk-wasm32-wasi is installed but is missing icechunk.wasm32-wasi.wasm.',
    )
  }
