...
async function create(path: string): Promise<void> {
  ...
  await session1.store.set('zarr.json', encoder.encode(JSON.stringify({ zarr_format: 3, node_type: 'group' })))
...
async function listCommits(path: string, branch: string, count: number | undefined): Promise<void> {
  const storage = await storageForPath(path)
  const repo = await Repository.open(storage)
  const commits = await repo.ancestry({
    branch,
  })

  const limit = count ?? commits.length
...
async function main(): Promise<void> {
  ...
  switch (command) {
    ...
    case 'list-commits':
      if (!args[0] || !args[1]) {
        console.error('Usage: main.ts list-commits <path> <branch>')
        process.exit(1)
      }
      await listCommits(args[0], args[1], args[2] ? Number(args[2]) : undefined)
      break
    default:
