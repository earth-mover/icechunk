...
use icechunk::{
    ...
    storage::{
        azure_blob::{AzureBlobConfig, AzureBlobStorage, AzureStorageCredentials, Location},
        StorageResult,
    },
    Storage,
};
...
async fn mk_storage() -> StorageResult<AzureBlobStorage> {
    AzureBlobStorage::new_azure_blob_store (
        "test_blob_storage__".to_string() + Utc::now().to_rfc3339().as_str(),
        &AzureBlobConfig {
            cloud_location: Some(Location::Emulator("127.0.0.1".to_string(), 10000)),
...
pub async fn test_snapshot_write_read() -> Result<(), Box<dyn std::error::Error>> {
    ...
    use futures::stream;
    storage.delete_snapshots(stream::once(async { id.clone() }).boxed()).await?;
    Ok(())
}
...
pub async fn test_manifest_write_read() -> Result<(), Box<dyn std::error::Error>> {
    ...
    use futures::stream;
    storage.delete_manifests(stream::once(async { id.clone() }).boxed()).await?;
    Ok(())
}
...
pub async fn test_chunk_write_read() -> Result<(), Box<dyn std::error::Error>> {
    ...
    use futures::stream;
    storage.delete_chunks(stream::once(async { id.clone() }).boxed()).await?;
    Ok(())
}
