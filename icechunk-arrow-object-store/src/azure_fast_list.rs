//! Azure List Blobs pages (raw-string `prefix`, 5000 blobs per page) scanned for
//! `<Content-Length>`. `SharedKey` signing follows `object_store`; azurite checks it in CI.
...
/// `Refreshable` refreshes and retries once on an authentication-expiry error.
/// Authorization errors are fatal, because a refresh cannot fix a missing role.
#[derive(Debug)]
...
/// The auth for one request, after any refresh.
#[derive(Debug)]
enum ConcreteAuth {
    Anonymous,
    Sas(String),
...
impl AzureLister {
    pub(crate) fn new(
        ...
        // Same as object_store: the emulator gets path-style `{account}/{container}`;
        // a custom endpoint keeps its path and gets `/{container}`.
    ...
    /// The returned credential is the one an expiry response invalidates.
    async fn resolve(
        &self,
    ...
    /// Query values are percent-decoded and sorted by name.
    fn string_to_sign(
        &self,
        date: &str,
        list_prefix: &str,
...
/// `Hard` holds an error status open so the caller can refresh credentials or fail.
enum PageResult {
    Attempt(PageAttempt),
...
impl ListPageFetcher for AzureLister {
    async fn attempt_page(&self, list_prefix: &str, token: Option<&str>) -> PageAttempt {
        ...
        // Refresh inside the attempt, so the engine does not read a token rotation
        // as congestion. Authorization errors are fatal and keep the Azure error code.
...
/// Azure markers for an expired or invalid SAS, bearer or account key.
...
/// The `x-ms-error-code` header and a body snippet.
...
enum ScanOutcome {
    ...
    /// The page had a `NextMarker` that was too long or unterminated. Retry, because
    /// ending the listing here would silently under-count.
    ContinuationLost,
}
...
/// Real markers are under 1 KB. A longer capture means a corrupt body.
...
/// Incremental page scanner. Only a non-empty `<NextMarker>` means truncated; the last
/// page has none, a self-closing one, or an empty one.
#[derive(Debug, Default)]
struct AzureScan {
    mode: Mode,
    total: u64,
...
impl AzureScan {
    ...
    /// A marker that was seen but not captured is an error, not the last page.
    ...
    /// Element text has no literal `<`, so a match invalidates only its own cached position.
