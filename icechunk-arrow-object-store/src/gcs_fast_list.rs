//! GCS JSON list pages with `fields=items(size),nextPageToken`: ~16 bytes per object
//! instead of ~330 in XML. The JSON API `prefix` is a raw string, so id splitting is valid.
...
/// `Refreshable` refreshes and retries once on a 401 that signals an expired token.
/// A 403 is fatal, because a refresh cannot fix it.
#[derive(Debug)]
...
pub(crate) struct GcsLister {
    ...
    /// settings applied.
    http: reqwest::Client,
    /// fake-gcs-server serves the JSON API at a custom endpoint the same way.
...
impl ListPageFetcher for GcsLister {
    async fn attempt_page(&self, list_prefix: &str, token: Option<&str>) -> PageAttempt {
        ...
        // as congestion. Other 401s and 403s are fatal and keep the GCS error text.
...
/// GCS markers for an expired or invalid OAuth token, the only 401 a refresh can fix.
...
/// The `WWW-Authenticate` challenge (it carries the expiry signal) and a body snippet.
...
enum ScanOutcome {
    ...
    /// The page had a `nextPageToken` that was too long or unterminated. Retry, because
...
/// Incremental page scanner. A key counts only as `"size"` or `"nextPageToken"` followed by
/// `:` and an opening quote, so a key name used as a value never matches.
#[derive(Debug, Default)]
struct JsonScan {
    mode: Mode,
    total: u64,
...
impl JsonScan {
    ...
    /// JSON has no `IsTruncated`: a `nextPageToken` that was seen but not captured is an error.
    ...
    /// An abandoned prelude can leave another pattern's cached position behind the
    /// resume point, so stale positions are searched again.
    ...
    /// Any unexpected byte abandons the match; scanning resumes at that byte.
    fn eat_prelude(
        &mut self,
        kind: Pat,
        mut seen_colon: bool,
