...
Icechunk is a strong foundation for storage of regulated and sensitive datasets — medical, financial, and personal. Its versioned history gives you a complete audit trail of every change, its storage inherits the encryption-at-rest and access controls of your object store (S3, GCS, Azure Blob), and its [expiration and garbage collection](../understanding/expiration.md) allows you to permanently delete data.

## What can be deleted

However not all (meta)data stored in Icechunk are permanently deletable. This is a design requirement in order to ensure that history, rebases and other core functions work properly.
...
- Paths, attributes, and zarr metadata in snapshots are removed only when you expire the snapshots that contain them

### Not Deletable

- Branch and tag names, commit messages, and the operation log are not removed by garbage collection. They live in the repo info file and its backups.
- [Transaction logs](../reference/spec.md#transaction-log-files) cannot be fully removed if they are a part of the branch's history ([design doc 16](https://github.com/earth-mover/icechunk/blob/main/design-docs/016-expired-transaction-logs.md)) so any [`move`](moving-nodes.md) operations can result in node path names being permanently persisted in storage.

## What to do

Sensitive data should **only** ever be stored in the array values. **Do not** put sensitive values in array or group names, attributes, dimension names, commit messages, or branch and tag names.
