...
@given(tree=zarr_trees(), data=st.data())
@settings(max_examples=50, deadline=None)
async def test_move_to_nonexistent_parent(tree, data):
    """Moving a node under a nonexistent parent should error, not silently delete."""
    arrays = set(tree.arrays())
    groups = set(tree.groups()[1:])
    existing_nodes = arrays | groups
    if not existing_nodes:
        return

    _, session, repo = make_stores(tree)
    session.commit("initial")

    source = data.draw(st.sampled_from(sorted(existing_nodes)))
    # Generate a name that doesn't exist as a group
    new_parent = data.draw(node_names.filter(lambda n, g=groups: n not in g))
    dest = f"{new_parent}/{source.split('/')[-1]}"

    note(f"tree: {zarr.open_group(repo.readonly_session('main').store, mode='r').tree()}")
    note(f"move {source!r} -> {dest!r} (parent {new_parent!r} does not exist)")

    session = repo.rearrange_session("main")
    session.move(f"/{source}", f"/{dest}")
    session.commit("move to nonexistent parent", allow_empty=True)

    ro = repo.readonly_session("main")
    ro_store = ro.store
    # The dest's parent should appear in list_dir of the root.
    # If the parent is missing from list_dir, the node was orphaned.
    dest_parent = dest.rsplit("/", 1)[0] if "/" in dest else dest
    root_entries = sorted(await collect_list_dir(ro_store, ""))
    assert dest_parent in root_entries or f"{dest_parent}/" in root_entries, (
        f"Moved {source!r} -> {dest!r} but parent {dest_parent!r} "
        f"not in root list_dir: {root_entries}"
    )
