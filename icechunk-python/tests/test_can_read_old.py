...
HTTP_VCC_URL_PREFIX = "http://example.com/"


def _write_http_vcc_repo(path: str, spec_version: ic.SpecVersion) -> None:
    """Create a repo with an HTTP VCC and persist its config to disk.

    Used by ``write_http_vcc_repo_v1`` / ``write_http_vcc_repo_v2`` to generate
    the committed fixture repos under ``tests/data/``.
    """
    shutil.rmtree(path, ignore_errors=True)
    config = ic.RepositoryConfig.default()
    # Use only opts (no headers) to match the 2.0.5 wire format so the fixture
    # remains representative of old repos that predate the headers field.
    vsc = ic.http_store(opts={"allow_http": "true"})
    container = ic.VirtualChunkContainer(HTTP_VCC_URL_PREFIX, vsc)
    config.set_virtual_chunk_container(container)
...
def write_http_vcc_repo_v2(path: str) -> None:
    """Write a V2 (spec v2) repo whose config contains an HTTP virtual chunk container.

    This is used to regenerate ``tests/data/test-repo-http-vcc-v2`` when run as a
    script (``python ./tests/test_can_read_old.py``).  The fixture committed to
    the repository was originally generated with icechunk==2.0.5, which did not
    have the ``headers`` field, so opening it exercises backward-compatible
    deserialization of the old V2 (FlexBuffers) wire format.
    """
    _write_http_vcc_repo(path, ic.SpecVersion.v2)


def write_http_vcc_repo_v1(path: str) -> None:
    """Write a V1 (spec v1) repo whose config contains an HTTP virtual chunk container.

    The config is stored as a standalone ``config.yaml`` file (YAML via
    ``serde_yaml_ng``), which is the code path this PR's serde change
    (``Http(HashMap<String,String>)`` → ``Http(HttpConfig)``) directly affects.

    This helper documents how ``tests/data/test-repo-http-vcc-v1`` was originally
    generated with icechunk==2.0.5 (``spec_version=SpecVersion.v1``).  V1 fixtures
    are **not** regenerated by the ``__main__`` block — they are permanent snapshots
    of the old on-disk format.
    """
    _write_http_vcc_repo(path, ic.SpecVersion.v1)


async def _do_test_http_vcc_repo(path: str) -> None:
    """Open a fixture repo containing an HTTP VCC and assert config survives round-trip."""
    repo = ic.Repository.open(storage=ic.local_filesystem_storage(path))
    config = repo.config
    vcc = config.get_virtual_chunk_container(HTTP_VCC_URL_PREFIX)
    assert vcc is not None, "HTTP VCC not found in config"
    assert isinstance(vcc.store, ic.ObjectStoreConfig.Http), (
        f"Expected ObjectStoreConfig.Http, got {type(vcc.store)}"
    )
    store_repr = repr(vcc.store)
    assert "allow_http" in store_repr, f"allow_http missing from repr: {store_repr}"

    # idempotency: save_config and re-open
    repo.save_config()
    repo2 = ic.Repository.open(storage=ic.local_filesystem_storage(path))
    config2 = repo2.config
    vcc2 = config2.get_virtual_chunk_container(HTTP_VCC_URL_PREFIX)
    assert vcc2 is not None, "HTTP VCC missing after save_config round-trip"
    assert isinstance(vcc2.store, ic.ObjectStoreConfig.Http)
    store_repr2 = repr(vcc2.store)
    assert "allow_http" in store_repr2, (
        f"allow_http missing after round-trip: {store_repr2}"
    )


async def test_icechunk_can_read_old_http_vcc_repo_v2() -> None:
    """V2 repo (config in binary ``repo`` file via FlexBuffers) written by
    icechunk==2.0.5 — exercises the FlexBuffers deserialization path."""
    await _do_test_http_vcc_repo("./tests/data/test-repo-http-vcc-v2")


async def test_icechunk_can_read_old_http_vcc_repo_v1() -> None:
    """V1 repo (config in ``config.yaml`` via serde_yaml_ng) written by
    icechunk==2.0.5 — exercises the YAML deserialization path that this PR's
    serde change (``Http(HashMap<String,String>)`` → ``Http(HttpConfig)``) affects."""
    await _do_test_http_vcc_repo("./tests/data/test-repo-http-vcc-v1")
...
if __name__ == "__main__":
    ...
    write_http_vcc_repo_v2("./tests/data/test-repo-http-vcc-v2")
