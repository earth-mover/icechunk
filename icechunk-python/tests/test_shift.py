...
async def test_reindex_using_function() -> None:
...
async def test_shift_left() -> None:
    ...
    session = repo.writable_session("main")
    root = zarr.group(store=session.store, overwrite=False)
    element_shift = session.shift_array("/array", (-4,))
    array = cast("zarr.Array[Any]", root["array"])
    ...
    np.testing.assert_equal(array[0:42], np.arange(8, 50))
    # Last 8 elements retain stale data (original values at those positions)
    np.testing.assert_equal(array[42:], np.arange(42, 50))
...
async def test_shift_right() -> None:
    ...
    # First 8 elements retain stale data (original values at those positions)
...
async def test_roll_left() -> None:
    """Test roll_array shifting left — circular buffer behavior."""
    repo = ic.Repository.create(
        storage=ic.in_memory_storage(),
    )
    session = repo.writable_session("main")
    ...
    session = repo.writable_session("main")
    root = zarr.group(store=session.store, overwrite=False)
    # Roll left by 2 chunks (4 array elements)
    session.roll_array("/array", (-2,))
    array = cast("zarr.Array[Any]", root["array"])
...
async def test_roll_right() -> None:
    """Test roll_array shifting right — circular buffer behavior."""
    repo = ic.Repository.create(
        storage=ic.in_memory_storage(),
    )
    session = repo.writable_session("main")
    ...
    session = repo.writable_session("main")
    root = zarr.group(store=session.store, overwrite=False)
    # Roll right by 2 chunks (4 array elements)
    session.roll_array("/array", (2,))
    array = cast("zarr.Array[Any]", root["array"])
...
async def test_resize_then_shift_right() -> None:
    """Test manual resize + shift for shifting right with no data loss."""
    repo = ic.Repository.create(
        storage=ic.in_memory_storage(),
    )
    session = repo.writable_session("main")
...
async def test_shift_2d() -> None:
    ...
    # First 2 rows retain stale data (same as original rows 0-1)
...
async def test_roll_2d() -> None:
    """Test roll_array on a 2D array."""
    repo = ic.Repository.create(
        storage=ic.in_memory_storage(),
    )
    session = repo.writable_session("main")
    ...
    session = repo.writable_session("main")
    root = zarr.group(store=session.store, overwrite=False)
    # Roll by (1, 1) - shift down and right by one chunk each
    session.roll_array("/array", (1, 1))
    array = cast("zarr.Array[Any]", root["array"])
    ...
    # 12 13 14 15
    #
    # After (1,1) roll (shift chunks, not elements):
...
async def test_shift_persists_after_commit() -> None:
    ...
    # Last 4 elements retain stale data
