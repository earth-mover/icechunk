...
@pytest.mark.filterwarnings("ignore:datetime.datetime.utcnow")
async def test_write_minio_virtual_refs_async() -> None:
    prefix = str(uuid.uuid4())
    etags = write_chunks_to_minio(
        [
            (f"{prefix}/chunk-1", b"first"),
...
async def test_public_virtual_refs_async(
    tmpdir: Path,
    container_type: str,
    url_prefix: str,
    store_config: ObjectStoreConfig.S3 | ObjectStoreConfig.Http,
...
async def test_error_on_nonexisting_virtual_chunk_container_async() -> None:
    repo = await Repository.open_or_create_async(
        storage=in_memory_storage(),
    )
    session = await repo.writable_session_async("main")
    ...
    await store.set_virtual_refs_async(
        array_path="/",
        validate_containers=False,
        chunks=[
            VirtualChunkSpec(
...
async def test_error_on_non_authorized_virtual_chunk_container_async() -> None:
    store_config = local_filesystem_store("/foo")
    container = VirtualChunkContainer("file:///foo", store_config)
    config = RepositoryConfig.default()
    config.set_virtual_chunk_container(container)
...
async def test_cannot_write_invalid_urls_async() -> None:
